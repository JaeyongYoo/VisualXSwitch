...
elementmap.xml: click-buildtool $(srcdir)/click-mkelemmap always
	echo $(DRIVERS)  analysis app aqm ethernet icmp ip local simple standard tcpudp test threads wifi $(EXTRA_PROVIDES) | $(top_builddir)/click-buildtool findelem -r "$(PROVISIONS) $(DRIVERS)" -p $(top_srcdir) $(FINDELEMFLAGS) | $(PERL) $(top_srcdir)/click-mkelemmap -r "$(PROVISIONS)" -t "$(DRIVERS)" -p $(top_srcdir) -Iinclude -s "`cd $(top_srcdir) && pwd`" $(MKELEMMAPFLAGS) > elementmap.xml
always:
	@:
...
config.status: $(srcdir)/configure
	$(SHELL) $(srcdir)/configure  '--enable-local' '--enable-userlevel' '--enable-standard' '--enable-ethernet' '--enable-wifi' '--disable-linuxmodule' '--enable-app' '--enable-aqm' '--enable-tcpudp' '--enable-ip' '--enable-icmp'
